...
import ImageOrPlaceholderDescription from "../ImageOrPlaceholderDescription.vue";
...
<template>
  <div class="border-b border-gray-800 movie-info" @keydown.esc="isOpenVideo = false">
    <div class="container flex flex-col px-4 py-16 mx-auto md:flex-row">
      <div class="flex-none">
        <image-or-placeholder :result="movie" size="w400" my_key="poster_path" />
      </div>
      <div class="md:ml-24">
        ...
        <div class="flex flex-wrap items-center mt-4 text-sm text-gray-700 dark:text-gray-400">
          ...
          <span class="mx-2">|</span>
          <span>{{ getMovieGenres(genres, pluckArray(movie.genres, "id")) }} </span>
        </div>
        <div class="flex-none">
          <image-or-placeholder-description v-bind:descripcion="movie['overview']" :images="movieImages" :res="movie" />
        </div>
        <div class="mt-12">
          <h4 class="font-semibold text-white">Direccion</h4>
          <div class="flex mt-4">
        ...
        <div class="mt-12" v-if="movieVideo">
          <button @click="isOpenVideo = true"
            ...
            <span class="ml-2">Ver ahora</span>
          </button>
          <div>
            <youtube-video-player @hideVideo="isOpenVideo = false" :videoKey="movie.videos.results[0].key"
              v-if="isOpenVideo" />
          </div>
        </div>
      </div>
